...
const buildInsightPayload = async (userId) => {
  ...
  const [locationAverages, timeline, scatterData, totals] = await Promise.all([
    ...
    Prediction.find(matchStage, { area_sqft: 1, predicted_price: 1, location: 1, created_at: 1 })
      .sort({ created_at: 1 })
      .lean(),
    Prediction.aggregate([
      { $match: matchStage },
...
export const postPrediction = async (req, res, next) => {
  try {
    const prediction = await createPrediction(req.body, req.user._id);
    res.status(201).json({
      success: true,
      data: prediction
    });
...
export const getInsights = async (req, res, next) => {
  try {
    const insights = await buildInsightPayload(req.user._id);
    res.json({
      success: true,
      data: insights
    });
